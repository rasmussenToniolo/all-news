...
export const NEWS_API_ADVANCED_URL = (data: advancedData) => `https://newsapi.org/v2/everything?${data.titleOnly ? 'qInTitle=' : 'q='}${data.query || ''}${data.exact ? `"${data.exact}"` : ''}${data.mustWords?.map(word => `+${word}`) || ''}${data.skipWords?.map(word => `-${word}`) || ''}${data.onlyDomains ? `&domains=${data.onlyDomains.join(',')}` : ''}${data.skipDomains ? `&excludeDomains=${data.skipDomains.join(',')}` : ''}${data.lang ? `&language=${data.lang}` : ''}${data.sort ? `&sortBy=${data.sort}` : ''}${data.date ? `${data.date.from ? `&from=${data.date.from}` : ''}${data.date.to ? `&to=${data.date.to}` : ''}` : ''}&apiKey=${NEWS_API_KEY}`;
