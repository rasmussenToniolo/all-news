...
export const Article = (props: any) => {
  ...
  return(
    <div className={`article priority-${props.article.priority}`}>
      ...
      <p className="article__date">{date.getDate()}/{date.getMonth()}/{date.getFullYear()} - {date.getHours()}:{`${date.getMinutes()}`.length === 1 ? `0${date.getMinutes()}` : date.getMinutes()}</p>

      {props.article.urlToImage ?
        // Show loading icon while waiting for image to load
        <div className="article__image">
          <div onClick={() => props.onImageExpand(props.article.urlToImage)} className="article__image-btn">
          ...
          <img src={props.article.urlToImage} alt={`Image of '${props.article.title}'`} className="article__image-img" />
        </div>
      : ''}

      <p className="article__description">{props.article.description}</p>

      <a href={props.article.url} target="_blank" className="article__full-article-btn">
        See full article
